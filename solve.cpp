...
public:
	...
	void givenlevel(node<T> *, int, std::ofstream&, std::vector<Triplet<T>>&);
	void levelorder(node<T> *, std::ofstream&, std::vector<Triplet<T>>&);
...
void BST<T>::givenlevel(node<T> *ptr, int level, std::ofstream& solution,
	std::vector<Triplet<T>>& sol) {

	if (ptr == NULL)
		return;
	if (level == 1) {
		solution << ptr->info.x << " " << ptr->info.y << " " << ptr->info.z << std::endl;
		sol.push_back(ptr->info);
	}
	else if (level > 1)
	{
		givenlevel(ptr->left, level - 1, solution, sol);
		givenlevel(ptr->right, level - 1, solution, sol);
	}
...
void BST<T>::levelorder(node<T> *ptr, std::ofstream& solution,
	...
	for (int i = h; i >= 1; i--) {
		BST::givenlevel(ptr, i, solution, sol);
	}
...
T Jacobian_cost(const std::vector<Triplet<T>>& Jac,
	...
	std::vector<std::vector<Cost_Triplet<T>>>& CJ_HEU) {
	...
	for (unsigned int j = 0; j < p; j++) {
		for (int i = j; i >= 0; i--) {
			if (i == j) {
				...
				CJ_HEU[j][i].cost = 0;
				CJ_HEU[j][i].split_pos = 0;
				CJ_HEU[j][i].dim = Jac[j];
			}
			else {
				for (int k = i + 1; k <= j; k++) {
				...
				std::vector<Triplet<T>> sub_Jac;
				std::vector<int> indices;
				for (int r = i; r <= j; r++) {
					sub_Jac.push_back(Jac[r]);
				}
				typename std::vector<Triplet<T>>::iterator result;
				result = std::min_element(sub_Jac.begin(), sub_Jac.end(),
					[](const Triplet<T>& a, const Triplet<T>& b) {return a.y < b.y; });
				T n_s = sub_Jac[std::distance(sub_Jac.begin(), result)].y;

				auto itr = sub_Jac.begin();
				while ((itr = std::find_if(itr, sub_Jac.end(),
					[n_s](const Triplet<T>& v) {return v.y == n_s; }))
					!= sub_Jac.end())
				{
					indices.push_back(std::distance(sub_Jac.begin(), itr) + i);
					itr++;
				}

				CJ_HEU[j][i].dim = CJ[j][i].dim;
				if (indices[0] == i) {
					CJ_HEU[j][i].split_pos = indices[0] + 1;
				}
				else {
					CJ_HEU[j][i].split_pos = indices[0];
				}
...
void Hessian_cost(const std::vector<Triplet<T>>& Hess,
	...
	std::vector<std::vector<Cost_Triplet<T>>>& CH_HEU) {
	...
	for (unsigned int j = 0; j < p; j++) {
		for (int i = j; i >= 0; i--) {
			if (i == j) {
				...
				CH_HEU[j][i].cost = 0;
				CH_HEU[j][i].split_pos = 0;
				CH_HEU[j][i].dim = Hess[j];
			}
			else {
				for (int k = i + 1; k <= j; k++) {
...
template<typename T>
T Heuristic_Bracket(std::vector<std::vector<Cost_Triplet<T>>>& CJ_HEU,
	std::vector<std::vector<Cost_Triplet<T>>>& CH_HEU,
	const std::vector<Triplet<T>>& heuristic_sol) {

	T p = CH_HEU.size();

	for (auto itr : heuristic_sol) {
		CJ_HEU[itr.x][itr.z].cost = CJ_HEU[itr.x][itr.y].cost +
			CJ_HEU[itr.y - 1][itr.z].cost + CJ_HEU[itr.x][itr.y].dim.x *
			CJ_HEU[itr.y - 1][itr.z].dim.x * CJ_HEU[itr.y - 1][itr.z].dim.y;

		CH_HEU[itr.x][itr.z].cost = CJ_HEU[itr.x][itr.y].cost + CH_HEU[itr.y - 1][itr.z].cost +
			CJ_HEU[itr.x][itr.y].dim.x * CH_HEU[itr.y - 1][itr.z].dim.x *
			CH_HEU[itr.y - 1][itr.z].dim.y * CH_HEU[itr.y - 1][itr.z].dim.z +
			CH_HEU[itr.x][itr.y].cost + CJ_HEU[itr.y - 1][itr.z].cost +
			CH_HEU[itr.x][itr.y].dim.x * CJ_HEU[itr.y - 1][itr.z].dim.x *
			CJ_HEU[itr.y - 1][itr.z].dim.y * (CH_HEU[itr.x][itr.y].dim.y +
				CJ_HEU[itr.y - 1][itr.z].dim.y);

	}

	return CH_HEU[p - 1][0].cost;
...
int main(int argc, char* argv[]) {
	...
	// Costs for Jacobian and Hessian vector
	...
	std::vector<std::vector<Cost_Triplet<T>>> CJ_HEU(p, std::vector<Cost_Triplet<T>>(p));
	std::vector<std::vector<Cost_Triplet<T>>> CH_HEU(p, std::vector<Cost_Triplet<T>>(p));
	...
	Jacobian_cost(Jac, CJ, CJ_HEU);
	Hessian_cost(Hess, CJ, CH, CH_HEU);
	...
	bst_CH.levelorder(root, solution, sol);
	bst_CH.clear();
	sol.clear();
	solution.close();
	...
	std::vector<Triplet<T>> heuristic_sol;
	std::ofstream heuristic_solution;
	heuristic_solution.open("heuristic_solution.txt");
	BST<T> bst_heuristic;
	node<T> *heuristic_CH;
	heuristic_CH = new node<T>;
	heuristic_CH->info.x = size - 1;
	heuristic_CH->info.y = CJ_HEU[size - 1][0].split_pos;
	heuristic_CH->info.z = 0;
	node<T> *root_heuristic = NULL;
	root_heuristic = bst_heuristic.insert(root_heuristic, heuristic_CH);
	Bracketing_structure(CJ_HEU, size, bst_heuristic, root_heuristic);
	bst_heuristic.levelorder(root_heuristic, heuristic_solution, heuristic_sol);

	T heuristic_cost = Heuristic_Bracket(CJ_HEU, CH_HEU, heuristic_sol);
	
	bst_heuristic.clear();
	heuristic_sol.clear();
	heuristic_solution.close();
	
	// Write the results
	std::cout << "left bracketing fma = " << left_cost << std::endl;
	std::cout << "right bracketing fma = " << right_cost << std::endl;
	std::cout << "heuristic bracketing fma = " << heuristic_cost << std::endl;
